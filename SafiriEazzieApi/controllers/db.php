    ...
    class DatabaseHandler {
        ...
        protected function update ($entityName, $updateModel, $keyModel){
            try{
                ...
                $db->AutoExecute($entityName, $updateModel, 'UPDATE', $sqlStatement);
                if($db->errorMsg() == null){
        ...
        protected function search ($searchModel){
            ...
            if($dbRes[0] == 1){
                ...
                $sqlStatement =  $this->paginateSQL($sqlStatement, $searchModel);
                $dbRes = $this->executeStatement($sqlStatement);
                if($dbRes[0] == 1){
                    if(isset($searchModel["associateModel"])){
                        $xitRes = array(1, 'Found ' . count($dbRes[2]) . ' items.', $this->associate($dbRes[2], $searchModel["associateModel"]), $this->makePages($sqlStatement, $searchModel));
                    }else{
                        $xitRes = array(1, 'Found ' . count($dbRes[2]) . ' items.', $dbRes[2], $this->makePages($sqlStatement, $searchModel));
                    }
                    return $xitRes;
                }
            } 
        ...
        private function makePages ($sqlStatement, $searchModel) {
            $xitRes = array();
            if(isset($searchModel["pageNo"]) && isset($searchModel["perPage"])){
                $pageNo = $searchModel["pageNo"];
                $perPage = $searchModel["perPage"];
                $sqlString = '';
                $stuff = explode(', ', $sqlStatement);
                $sqlString .= 'SELECT COUNT(' . explode(' ', $stuff[0])[1] . ')';
                $stuff_ = explode(' ', $stuff[count($stuff)-2]);
                $sqlString .= ' ' . $stuff_[3];
                $sqlString .= ' ' . $stuff_[4];
                $dbRes = $this->fetchItem(null, null, $sqlString);
                if($dbRes[0] == 1){
                    $itemsCount = $dbRes[2];
                    if(($pageNo * $perPage) > intval($itemsCount)){
                        $xitRes["range"] = $pageNo . ' - ' . intval($itemsCount) . ' of ' . intval($itemsCount);
                    }else{ 
                        $xitRes["range"] = $pageNo . ' - ' . ($pageNo * $perPage) . ' of ' . intval($itemsCount);
                    }
                    $xitRes["pages"] = array();
                    if(($itemsCount % intval($perPage)) > 0){
                        $itemsCount = ($itemsCount/intval($perPage)) + 1;
                    }else{
                        $itemsCount = ($itemsCount/intval($perPage));
                    }
                    for($page = 1; $page <= $itemsCount; $page ++){
                        if($page == $pageNo){
                            array_push($xitRes["pages"], array("page"=>$page, "state"=>true));
                        }else{
                            array_push($xitRes["pages"], array("page"=>$page, "state"=>false));
                        }
                    }
                }
            }
        ...
        private function paginateSQL ($sqlStatement, $searchModel){
            if(isset($searchModel["pageNo"]) && isset($searchModel["perPage"])){
                $pageNo = $searchModel["pageNo"];
                $perPage = $searchModel["perPage"];
                $startPoint = ((($pageNo * $perPage) - $perPage));
